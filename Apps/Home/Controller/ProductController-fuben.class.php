...
class ProductController extends IController {
    ...
    public function index(){
        ...
        $side_Nav=$this->CategoryModel->where(array('pid'=>$catid))->order('sort asc,id desc')->select();
        foreach($sidebar as $key => $val){
          $side_Nav[$key]=$val;
          $side_Nav[$key]['url']=U('product/index',array('catid'=>$val['id']));
        }
        dump($side_Nav[$key]['url']);
